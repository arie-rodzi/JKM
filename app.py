...
with c3: kpi("Outcome Index", f"{avg_out:.1f}", "WHODAS + Wellbeing + WAI + CSQ-8 + PCL-5")
with c4: kpi("Dropout", f"{dropout:.1f}%", "Simulation indicator")
with c5: kpi("T3 Follow-up", f"{follow:.1f}%", "Maintenance readiness")

tabs = st.tabs(["01 Overview", "02 Negeri & Zon", "03 T1-T2-T3 Outcomes", "04 SEM", "05 Model Results", "06 RE-AIM + CMO", "07 Kualitatif", "08 Policy", "09 Scenario", "10 Data"])
...
with tabs[1]:
    ...
    metrics = ["Client_Outcome_Index", "Service_Quality", "Service_Mechanism", "Organizational_Capacity", "Access_Responsiveness", "CSQ8_Satisfaction", "WAI_Alliance", "waiting_days"]
